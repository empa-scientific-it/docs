...
<CustomBreadcrumbs
  crumbs={[
    ...
    { label: 'Register a new user', link: '/documentation/openbis/empa-specific/for_admins/add_user' },
  ]}
/>
...
1. Now you need to choose the group ending in `Full` in the `groups` table. In the screenshot above, you can see two groups:
  ...
  **By choosing a group the user will get automatically user rights assigned in openBIS. Check the ETH documentation for the default rights [here](https://openbis.readthedocs.io/en/latest/user-documentation/general-admin-users/admins-documentation/user-registration.html#default-roles-assigned-in-eln)**.
  ...
  _For your information: On the left side in the bottom box called `Managers` the instance admins of your openBIS are listed, so they have the right to add users here on the right side (Managers are added by ICT when a new instance admin is requested via [support forms](/documentation/openbis/getting-started/support/))_.
...
1. Ask the user to try and login. If the login fails, make sure at least 24 hours elapsed since you added them. If this is the case, use the [support form for bugs](/documentation/openbis/getting-started/support/) or contact us via [e-mail](mailto:openbis-support@empa.ch).
...
```mermaid
  graph TD
    A[User 👨‍💻] --Request--> B[openBIS server 🖥️]
    B --Redirect--> C[AAI 🖥️]
    C --Ask for authentication--> A
    A --Authenticate--> C
    C --Access--> B
    subgraph Empa
      A -.- D[LDAP 🖥️]
    end
      D -.Groups via Synchronization Task..- B
```
