...
CSCS provides and maintains the [Container Engine](https://docs.cscs.ch/software/container-engine/) (CE), a set of tools to help users run containerized applications exploiting all the features of the underlying hardware.
...
The last step before running a container from the image we built is creating an Environment Definition File (EDF).
Such file tells the CE where to look for the image to run, which folders to mount inside the container, and [many other options](https://docs.cscs.ch/software/container-engine/edf/) we might need/want to customize.
...
- [Building container images](https://docs.cscs.ch/build-install/containers/)
- [The Container Engine](https://docs.cscs.ch/software/container-engine/)
